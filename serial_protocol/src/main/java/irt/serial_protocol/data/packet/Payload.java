...
import irt.serial_protocol.data.DacValue;
import irt.serial_protocol.data.DeviceId;
...
public class Payload {
	...
	public Payload(byte parameterId, byte[] buffer){
		this.parameterHeader = new ParameterHeader(parameterId);
		if(buffer!=null){

			if(buffer.length > Short.MAX_VALUE)
				logger.error("Buffer is to long.");
			else
				parameterHeader.setSize((short) buffer.length);
		}

		this.buffer = parameterHeader.getSize()!=0 ? buffer : null;
	}
